...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, MvcRequestMatcher.Builder mvc) throws Exception {
        http
                ...
                .csrf(csrf -> csrf.disable()) // CSRF protection is needed only for
                ...
                .authorizeHttpRequests((authz) -> authz
                        .requestMatchers(mvc.pattern(HttpMethod.GET, "/api/v1/people/**")).hasAnyAuthority("ROLE_ADMIN", "ROLE_USER")
    ...
    // http
    // .cors(cors -> cors.disable())
    // .csrf(csrf -> csrf.disable()) // CSRF protection is needed only for browser
    // based attacks
    ...
    // // .requestMatchers("/api/v1/people/**").permitAll()
    // // );
    // // .authenticationProvider(authenticationProvider()); //specifies the
    // authentication provider for HttpSecurity
